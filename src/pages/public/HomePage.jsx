...
function DropCard({ product }) {
  ...
  const handleQuickAdd = (e) => {
    e.preventDefault()
    if (!hasStock || !firstAvailableSize) { toast.error(t.toastOutOfStock); return }
      ...
      <div className="relative w-full aspect-[3/4] overflow-hidden bg-surface-container mb-3">
        ...
        {!hasStock && (
          <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
            <span className="stitch-label text-outline">{t.outOfStock}</span>
          </div>
        )}
        {hasStock && (
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <section className="px-6 lg:px-16 xl:px-24 py-16 lg:py-24">
        <h3 className="stitch-label text-center mb-10 lg:mb-14 tracking-[0.4rem]">{t.categoriesTitle}</h3>
