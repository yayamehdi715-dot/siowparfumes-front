        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <select value={status} onChange={handleChange}
              className={`bg-transparent border border-white/10 text-xs font-label
                          ...
                          focus:border-white/20 appearance-none cursor-pointer
                          ${currentOpt?.color}`}>
          ...
          {dirty && (
            <button onClick={handleSave} disabled={saving}
              className="w-7 h-7 bg-[#8C495F] flex items-center justify-center
                         hover:bg-[#a05570] transition-colors disabled:opacity-50">
              {saving
